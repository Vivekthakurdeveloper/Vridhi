...
- **Google Drive** — OAuth connect, folder pick, "Sync Now". Full trace in §5.

Gmail exists only as a catalog entry with `status: "not_implemented"` (`routers/workspace.py`, `_connector_catalog`).
...
Only ingestion and Drive sync are async. Two interchangeable queue backends (`services/queue.py`):
...
Job types are dispatched in `worker/consumer.py:_dispatch_job`: `drive_sync` → `process_drive_sync_job`, everything else → `process_ingest_job`.
...
| **OpenSearch** | one doc per chunk: `content`, `title`, denormalised ACL fields (`tenant_id`, `visibility`, `uploaded_by_user_id`, `granted_user_ids`), and a 384-dim `knn_vector` | Retrieval only — it's a *derived* index, rebuildable from Postgres + S3 |
...
2. `job.progress_total = len(files)`, then per file `_upsert_drive_file`:
   - Skip if `file_cursors[external_id] == modifiedTime` **and** the doc is already `ready` → `progress_skipped++`.
...
- **It's a point-in-time snapshot.** Revoking someone in Drive doesn't revoke them in Vridhi until the file's `modifiedTime` changes *and* a sync runs. Sharing changes typically don't bump `modifiedTime`, so **ACL revocations will routinely be missed**. This is the single most important limitation of the Drive integration.
...
- **Deletions don't propagate.** A file deleted or unshared in Drive stays in Vridhi and stays answerable forever; there's no tombstone pass.
...
| `GOOGLE_DRIVE_MODE` | `mock` | 3 folders / 3 hardcoded `.txt` files with fake permissions, defined inline at `services/drive.py:40-97`. OAuth start redirects to the app's own callback. |
...
**1. Deleting a document does not remove it from OpenSearch.** `DocumentService.delete_document` (`documents.py:354`) sets `status=deleted` + `deleted_at` and commits. It never calls `delete_by_document` — the only caller is the *worker*, on re-ingest (`ingest.py:87`). The OpenSearch mapping has no deletion field and `acl_filter` has no corresponding clause. **Consequence: a deleted document's chunks remain retrievable and citable by `/v1/search` and `/v1/chat` indefinitely.** Postgres-backed views (documents list, preview) correctly hide it, so this is invisible in the UI until someone asks a question. This is a data-retention and potentially a compliance bug, and it's the first thing I'd fix.

**2. ACL changes don't reach the index either.** `visibility`, `uploaded_by_user_id` and `granted_user_ids` are frozen into each OpenSearch doc at ingest. Any future "change who can see this" feature will silently do nothing to retrieval unless it triggers re-indexing. Combined with §5's snapshot problem, revocation is broadly unreliable. The clean fix is to stop denormalising ACLs into the index and instead post-filter against Postgres — or at minimum add a re-index-on-ACL-change path.

**3. Two implementations of one authorization rule.** `can_access` (imperative, `documents.py:119`), `accessible_filter` (SQLAlchemy, `:149`), and `acl_filter` (OpenSearch DSL, `retrieval.py:61`) all encode the same policy in three languages. They agree today. They will drift. If you touch visibility semantics, change all three and add a test that asserts they agree.
...
Fix #1 (index cleanup on delete) — it's a small, well-bounded change with a real correctness payoff, and writing the test for it forces you to touch the ACL trio in #3, which is the best way to learn this codebase.
