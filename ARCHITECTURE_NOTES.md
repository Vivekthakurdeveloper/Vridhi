...
Gmail is a third real connector (OAuth, attachment sync, History-based deletion; see §5.2 and §8), and Google Chat is a fourth (OAuth, one document per thread, full-relist deletion since Chat has no History-API equivalent; see §5.4). Both also appear in the catalog (`routers/workspace.py`, `_connector_catalog`).
...
### 5.4 Google Chat integration (Piece 4B)

Same connector shape as Drive/Gmail (OAuth → connect → select → sync → automatic sync via the generic scheduler), but two structural choices set it apart from both.

**Per-admin OAuth only, no Domain-Wide Delegation.** Unlike the Workspace-Enterprise Groups sync (above, `WORKSPACE_ENTERPRISE_MODE`), Chat has no admin-impersonated service-account path — `ChatService.oauth_start_url` is a normal per-user OAuth-consent redirect (mock mode: the app's own callback, same convention as Drive/Gmail), and every sync runs with whichever admin's token connected the space. There is no Chat equivalent of `services/groups.py`'s DWD verification. Practically: if the connecting admin loses access to a space (leaves it, is offboarded), that space's syncs start failing with whatever the Chat API returns for a member no longer in the space — there is no separate super-admin credential to fall back on the way Groups has one.

**One `Document` per thread, not per message.** `worker/chat_sync.py:_upsert_chat_thread` groups a space's messages by `thread.name` (`services/chat_threads.py:group_messages_by_thread`), builds one flattened transcript (`format_thread_transcript`) per thread, and upserts a single `Document` (`source="chat"`, `external_id="spaces/<S>/threads/<T>"`) per thread — re-versioned on every sync that has new messages in that thread, exactly like a file getting a new `DocumentVersion` in Drive. Supported attachments get their own `Document` (`source="chat_attachment"`), one per attachment, same as Drive/Gmail's separate-document treatment of attachments.

**`DocumentGrant` per resolved space member, re-checked on every sync — even for threads with no new messages.** This is the same rule Piece 2 added to Drive's ACL recheck (§5.2, §5.3): `_run_chat_sync` calls `_fetch_space_members` and `resolve_member_grants` (email → active `OrganizationMember`, same fail-closed identity matching as Drive's — no Google `sub` linkage) once per space, then `_upsert_chat_thread` iterates **every thread the listing returned this run**, not just threads with new messages. For an unchanged thread it still deletes and re-adds that thread's `DocumentGrant` rows from the freshly-resolved member set, and — mirroring Drive's ACL-changed re-ingest — enqueues a re-ingest of the thread's current version only if the resolved grant set actually differs from what was stored (`member_grant_set_changed`), so the denormalised OpenSearch ACL fields (§3) get rewritten too. A user who leaves a space loses search/RAG visibility on that space's threads on the next sync, same latency caveat as Drive: sync-triggered, not push-triggered.

> **Known-risk / unverified in live mode:** `_fetch_space_members`'s live-mode path reads `member.get("email")` off each `Membership.member` (a Chat API `User` resource) returned by `spaces.members.list`. This is **unconfirmed against the real Chat API** — Google's own docs are ambiguous about whether `User.email` is populated on that resource for a Workspace-internal member, as opposed to requiring a separate lookup. Mock mode (`GOOGLE_CHAT_MODE=mock`) is unaffected, since `get_mock_members` returns email addresses directly from the fixture and never exercises this code path. If live testing shows `email` is absent or empty, the fallback is the same Admin SDK Directory lookup `services/groups.py` already performs for Workspace-Enterprise group sync (resolve the member's `User.name` resource id via `admin.directory.users.get`, not `spaces.members.list` alone) — not yet implemented here; this fix pass only documents the gap; it does not change `_fetch_space_members`'s code.

**No History-API equivalent, so deletion is always a full relist-and-diff, per space, per sync.** Gmail's `gmail_history_id` checkpoint (§5.2) has no Chat counterpart — the Chat API gives no changes-since-cursor stream. `_run_chat_sync` therefore always lists **every** message in a space (`_list_messages`, paginated, no cursor) to compute deletions, regardless of `incremental`; the cursor (`connection.config["chat_cursors"][space_id]`, the newest `createTime` seen) only narrows which messages count as *new* for the has-new-messages/re-ingest decision, not which are fetched. Deletion itself diffs the full per-space listing against stored, non-deleted `chat` documents for that space (`missing_external_ids`) and tombstones anything missing via `tombstone_many(..., reason=REASON_SOURCE_REMOVED)` — same fail-closed ordering as Drive/Gmail: a space listing that raises skips that space's deletion pass entirely rather than tombstoning on partial data. Cost-wise this means Chat sync is O(all messages in a space) every run, not O(new messages) — there is no way to make it cheaper without a Google-side checkpoint API that doesn't exist today.

**Scheduling.** Wired into the same generic scheduler as Drive/Gmail (`worker/scheduler.py:run_scheduler_tick` — `SyncJobType.chat_sync` is a third member of `_SYNC_JOB_TYPES`, and `google_chat`'s `start_sync` is called through the same `elif conn.connector_type == "google_chat":` branch), so it inherits the same due-rule, stale-job handling, `auto_sync_max_starts_per_tick` fairness, and five-consecutive-failures auto-pause as Drive/Gmail (§5.2) — no Chat-specific scheduling code exists.

**Mock mode.** `GOOGLE_CHAT_MODE=mock`: `MOCK_SPACES` (two spaces), `MOCK_MEMBERS` (per-space email lists — one space has zero members, used to prove a never-populated space's threads leak to nobody), `MOCK_MESSAGES` (three messages across two threads), with a `CHAT_MOCK_OVERRIDES_PATH` side-channel file (default `apps/api/.mock-chat-overrides.json`, `removed_message_ids`) mirroring Drive's and Gmail's override-file convention exactly. Covered by `scripts/smoke-phase-j.sh` (not run live in this environment — see §7).
...
API_URL=http://localhost:8000 ./scripts/smoke-phase-j.sh   # Google Chat mock OAuth→spaces→sync→per-member grants→full-relist deletion→auto-sync
```

(Phases e/f/h/i exist too but aren't re-listed here — this section was never kept exhaustive across every phase; see the script directory itself for the full set.)
